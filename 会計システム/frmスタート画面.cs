...
namespace AccountingSystem
{
    public partial class frmスタート画面 : Form
    {
        ...
        {
            var 勘定科目 = new ApplicationService.勘定科目.勘定科目情報サービス();
            foreach (var 取引科目 in 勘定科目.取引勘定科目リスト)
            {
        ...
        private void 画面に表示している伝票を登録する()
        {
            ...
            {
                画面ウェイト();
                ApplicationService.会計伝票.仕訳構築サービス 構築した仕訳 = 伝票仕訳を構築する();

                var 伝票構築サービス = new ApplicationService.会計伝票.会計伝票構築サービス();
                Domain.BusinessObject.会計伝票.伝票 保存する伝票 = 伝票構築サービス.伝票を構築する(dtp計上日.Value.Date, 構築した仕訳);

                var 会計伝票記帳サービス = new ApplicationService.会計伝票.会計伝票記帳サービス();
            ...
            catch (Exception ex)
            {
                ...
                foreach (var item in Controls)
                {
                    if (item is ctrl仕訳)
                    {
                        ...
                        {
                            case ctrl仕訳.貸借.借方:
                                借方合計金額 = 借方合計金額 + ApplicationService.共通.型変換サービス.文字列を金額に変換する(仕訳コントロール.txt金額.Text).金額;
                                break;
                            case ctrl仕訳.貸借.貸方:
                                貸方合計金額 = 貸方合計金額 + ApplicationService.共通.型変換サービス.文字列を金額に変換する(仕訳コントロール.txt金額.Text).金額;
                                break;
                        }
                    }
                }
        ...
        {
            if (ApplicationService.共通.型変換サービス.文字列を金額に変換する(仕訳コントロール.txt金額.Text).変換可能 == false)
            {
        ...
        /// </summary>
        /// <returns></returns>
        private ApplicationService.会計伝票.仕訳構築サービス 伝票仕訳を構築する()
        {
            var 伝票仕訳 = new ApplicationService.会計伝票.仕訳構築サービス();
            ...
            foreach (var item in Controls)
            {
                ...
                {
                    仕訳コントロール = (ctrl仕訳)item;
                    int 勘定科目コード = ApplicationService.共通.型変換サービス.コードと名称から勘定科目コードを抽出する(仕訳コントロール.cmb勘定科目.Text);
                    var 金額入力欄 = ApplicationService.共通.型変換サービス.文字列を金額に変換する(仕訳コントロール.txt金額.Text);
                    if (金額入力欄.変換可能 == false)
                    {
        ...
        {
            var 科目一覧 = new ApplicationService.勘定科目.勘定科目情報サービス();
        ...
        private void 伝票番号で検索する()
        {
            ...
            try
            {
                var 伝票検索 = new ApplicationService.会計伝票.会計伝票検索サービス();
        ...
        private void cmd勘定科目検索_Click(object sender, EventArgs e)
        {
            ...
            var 伝票検索 = new ApplicationService.会計伝票.会計伝票検索サービス();
            int 検索する勘定科目 = ApplicationService.共通.型変換サービス.コードと名称から勘定科目コードを抽出する(cmb検索する勘定科目.Text);
        ...
        {
            画面ウェイト();
            var 記録 = new ApplicationService.運用.動作記録サービス();
        ...
        private void cmd伝票訂正_Click(object sender, EventArgs e)
        {
            ...
            var 伝票検索サービス = new ApplicationService.会計伝票.会計伝票検索サービス();
            ...
            var 伝票記帳サービス = new ApplicationService.会計伝票.会計伝票記帳サービス();
