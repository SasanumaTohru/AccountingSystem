...
namespace AccountingSystem.ApplicationService
{
    public class 合計残高試算表構築サービス
    {
        ...
        public 合計残高試算表 合計残高試算表を取得する(会計年度 会計年度)
        {
            ...
            System.Collections.ObjectModel.ReadOnlyCollection<科目> m_取引勘定科目リスト = 勘定科目情報サービス.取引勘定科目リスト;
            ...
            foreach (var 勘定科目 in m_取引勘定科目リスト)
            {
                ...
                金額 借方金額 = 集計対象仕訳.貸借別合計金額を取得する(仕訳列.貸借区分リスト.借方);
                金額 貸方金額 = 集計対象仕訳.貸借別合計金額を取得する(仕訳列.貸借区分リスト.貸方);
